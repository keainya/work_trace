...
async function handleOAuthCallback() {
	...
	const redirectURI =
		localStorage.getItem("wt_oauth_redirect") || OAUTH_CONFIG.redirect_uri;
...
async function doRoute() {
	...
	try {
		...
		$main.innerHTML =
			'<div class="empty"><div class="icon">!</div><p>页面加载失败：' +
			escHtml(err.msg || "未知错误") +
			"</p></div>";
	}
	window.scrollTo(0, 0);
}
...
function renderHeader() {
	...
	if (currentUser) {
		d +=
			...
			"</div>";
		d +=
			'<a href="#/" onclick="closeDrawer()" class="' +
			(currentPage === "list" || currentPage === "detail" ? "active" : "") +
			'">工作项</a>';
		d += '<div class="drawer-divider"></div>';
		d +=
			'<button class="btn btn-outline btn-block btn-logout" onclick="logout();navigate(\'#/login\');closeDrawer()">退出登录</button>';
	} else {
...
async function renderList() {
	...
	async function loadItems() {
		...
		try {
			...
			if (items.length === 0) {
				grid.innerHTML =
					'<div class="empty"><div class="icon">&#x1F4CB;</div><p>暂无工作项</p></div>';
				return;
			}
...
async function renderDetail(itemId) {
	...
	try {
		...
		$main.innerHTML =
			'<div class="empty"><div class="icon">!</div><p>' +
			escHtml(err.msg || "工作项不存在") +
			"</p>" +
...
function expandDetailTextarea() {
	...
	overlay._keyHandler = (e) => {
		if (e.key === "Escape") collapseDetailTextarea();
	};
...
function checkDirty() {
	...
	const changed =
		...
		(start &&
			start.value !== toDatetimeLocal(currentItemOriginal.start_time)) ||
		(end && end.value !== toDatetimeLocal(currentItemOriginal.end_time)) ||
		(remind &&
			remind.value !== toDatetimeLocal(currentItemOriginal.remind_at)) ||
...
async function loadSubTasks(itemId) {
	try {
		...
		if (subs.length === 0) {
			el.innerHTML =
				'<div class="empty" style="padding:16px;"><p>暂无子任务</p></div>';
			return;
		}
		el.innerHTML =
...
async function addSubTask(itemId) {
	...
	if (!content) return;
	try {
		await apiPost("/api/work-items/" + itemId + "/sub-tasks", {
			content: content,
		});
		input.value = "";
...
async function toggleSubTask(itemId, subId) {
	try {
		await apiPost(
			"/api/work-items/" + itemId + "/sub-tasks/" + subId + "/toggle",
			{},
		);
		loadSubTasks(itemId);
	} catch (err) {
...
async function loadRecords(itemId) {
	...
	try {
		...
		if (records.length === 0) {
			list.innerHTML =
				'<div class="empty" style="padding:20px;"><p>暂无工作记录</p></div>';
		} else {
			list.innerHTML =
				'<div class="record-list" style="padding:0 0 12px 0;">' +
				records
					.map(
						(r) => `
		...
		list.innerHTML =
			'<div class="empty" style="padding:20px;"><p>加载失败</p></div>';
		list.style.display = "block";
		toggle.innerHTML = "工作记录 &#x25B2;";
		recordsLoaded = true;
	}
